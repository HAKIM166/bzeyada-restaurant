...
export default function AddonsModal({ item, close, addons }) {
  ...
  const updatePaid = (addon, qty) => {
    setSelectedPaid((prev) => {
      if (qty === 0) return prev.filter((x) => x.id !== addon.id);
      const exists = prev.find((x) => x.id === addon.id);
      if (exists)
        return prev.map((x) => (x.id === addon.id ? { ...x, qty } : x));
    ...
    <>
      ...
      <div
        className="
          ...
          w-[90%] max-w-[380px]
          bg-[#181818] text-white rounded-2xl shadow-xl 
      ...
      >
        ...
        <div className="flex justify-between items-center p-4 border-b border-white/10">
          ...
          <button
            onClick={close}
            className="text-gray-300 text-xl hover:text-white transition"
          >
            ✕
          </button>
        </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {addons.paid.map((addon) => {
                  const ex = selectedPaid.find((x) => x.id === addon.id);
                  const qty = ex?.qty || 0;
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <select
                          value={qty}
                          onChange={(e) =>
                            updatePaid(addon, Number(e.target.value))
                          }
                          className="
        ...
        <div className="p-4 border-t border-white/10">
          <button
            onClick={saveChanges}
            className="
              w-full py-3 bg-red-600 rounded-xl 
